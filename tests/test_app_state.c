...
// Click-to-talk: PRESS records the real ADC level, RELEASE/CLICK fire the
// READY-state start transition.  Tests use this instead of a raw PRESS.
static void start_ptt_click(uint64_t ts) {
    reduce_btn(APP_EV_KEY_PRESS, APP_BTN_UP, ts);
    reduce_btn(APP_EV_KEY_RELEASE, APP_BTN_UP, ts + 1);
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_UP, ts + 2);
}
...
// ---- HOME:● 单击进入 READY;▼ 单击=粘贴 / 长按=清空;▲ 无空闲语义 ----
static void test_home_nav(void) {
    ...
    assert(a && a->u.key_action.action == APP_KEY_PASTE);
...
// ---- READY:OK 单击=回车;▲ 按下=说话;▼ 单击=粘贴 / 长按=清空 ----
static void test_down_enter_clear(void) {
    ...
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 25);   // OK 单击 = 回车
    a = find_action(APP_ACT_SEND_KEY_ACTION);
...
// UP 的单击只负责启动语音;没有紧邻 PRESS 的裸 CLICK 不得产生动作。
static void test_up_taps_never_clear(void) {
    ...
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_UP, now + 3000);
    assert(!has_action(APP_ACT_SEND_KEY_ACTION));
    assert(on == 0);
    assert(s.state == APP_ST_READY);
}
...
// OK 长按也必须发送回车,不能再锁屏。
static void test_ok_long_sends_enter(void) {
    reset();
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);      // → READY
    reduce_btn(APP_EV_KEY_LONG_UP, APP_BTN_OK, now + 600);
    app_action_t *a = find_action(APP_ACT_SEND_KEY_ACTION);
    assert(a && a->u.key_action.action == APP_KEY_ENTER);
...
// 点击 UP 启动,任意键 PRESS 结束。
static void test_ptt_click_toggle(void) {
    reset();
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);      // → READY
    start_ptt_click(now + 20);
    assert(s.state == APP_ST_LISTENING);
    assert(has_action(APP_ACT_SEND_VOICE_START));

    reduce_btn(APP_EV_KEY_PRESS, APP_BTN_DOWN, now + 700);
...
static void test_tone_done_late_after_send(void) {
    ...
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);
    start_ptt_click(now + 20);    // 按下 #1:入 LISTENING,未开流
    assert(!has_action(APP_ACT_STREAM_START));
    // 松开:立即发送(≥ PTT_MIN_TALK_MS,否则算阈值误触被取消,见 test_ptt_click_toggle)
...
static void test_ptt_press_start_guards(void) {
    // 幽灵 PRESS 后补 CLICK:不启动会话
    reset();
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);          // READY
    reduce_btn_mv(APP_EV_KEY_PRESS, APP_BTN_UP, 2890, now + 20);
    reduce_btn(APP_EV_KEY_RELEASE, APP_BTN_UP, now + 21);
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_UP, now + 22);
    assert(s.state == APP_ST_READY);
    assert(!has_action(APP_ACT_SEND_VOICE_START));

    // 跨档幽灵 PRESS(598mV)后补 CLICK:同样不启动
    reset();
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);
    reduce_btn_mv(APP_EV_KEY_PRESS, APP_BTN_UP, 598, now + 20);
    reduce_btn(APP_EV_KEY_RELEASE, APP_BTN_UP, now + 21);
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_UP, now + 22);
    assert(s.state == APP_ST_READY);
    assert(!has_action(APP_ACT_SEND_VOICE_START));

    // 真 UP 单击:PRESS(3mV) -> RELEASE -> CLICK 启动会话
    reset();
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);
    reduce_btn_mv(APP_EV_KEY_PRESS, APP_BTN_UP, 3, now + 40);
    reduce_btn(APP_EV_KEY_RELEASE, APP_BTN_UP, now + 41);
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_UP, now + 42);
    assert(s.state == APP_ST_LISTENING);
    assert(has_action(APP_ACT_SEND_VOICE_START));

    // 不足 500ms 时任意键 PRESS:丢弃音频、voice.end、回 READY
    reduce_btn(APP_EV_KEY_PRESS, APP_BTN_DOWN, now + 442);
    assert(s.state == APP_ST_READY);
    assert(has_action(APP_ACT_STREAM_CANCEL));
    assert(has_action(APP_ACT_SEND_VOICE_END));

    // 足够时长后任意键 PRESS:正常进入 TRANSCRIBING
    reset();
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);
    ...
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_UP, now + 22);
    assert(s.state == APP_ST_LISTENING);
    reduce_btn(APP_EV_KEY_PRESS, APP_BTN_OK, now + 700);
    assert(s.state == APP_ST_TRANSCRIBING);
    assert(has_action(APP_ACT_STREAM_STOP));
    assert(has_action(APP_ACT_SEND_VOICE_END));
}
...
static void test_locked_keys_operate_screen_off(void) {
    ...
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_DOWN, now + 50);  // DOWN 单击:照常粘贴
    a = find_action(APP_ACT_SEND_KEY_ACTION);
    assert(a && a->u.key_action.action == APP_KEY_PASTE);
    assert(s.screen_on == false);

    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 70);    // OK 单击 = 回车
    a = find_action(APP_ACT_SEND_KEY_ACTION);
    assert(a && a->u.key_action.action == APP_KEY_ENTER);
    assert(s.locked == true);
    assert(s.screen_on == false);

    start_ptt_click(now + 80);     // 再按 UP:照常开录
    assert(s.state == APP_ST_LISTENING);
    assert(has_action(APP_ACT_SEND_VOICE_START));
...
static void test_panel_off_longest_paths(void) {
    // a. 面板断电 + ▲ 单击:PRESS 唤醒,RELEASE/CLICK 再启动语音。
    reset();
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);              // → READY
    reduce(APP_EV_TICK, s.last_key_ms + APP_IDLE_PANEL_OFF_MS + 1);  // 60s:面板断电
    assert(s.screen_on == false && s.panel_on == false);
    reduce_btn(APP_EV_KEY_PRESS, APP_BTN_UP, now + 10);              // 唤醒
    assert(has_action(APP_ACT_UI_PANEL_ON));
    assert(has_action(APP_ACT_UI_SCREEN_ON));
    reduce_btn(APP_EV_KEY_RELEASE, APP_BTN_UP, now + 11);
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_UP, now + 12);              // 启动语音
    assert(s.state == APP_ST_LISTENING);
    assert(has_action(APP_ACT_PLAY_TONE));
    assert(has_action(APP_ACT_SEND_VOICE_START));
    assert_action_order(APP_ACT_PLAY_TONE, APP_ACT_SEND_VOICE_START);
    ...
    // c. 面板断电 + 离线 UP 单击:PRESS 唤醒,CLICK 给 error 音和 toast。
    reset();
    s.link_up = false;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);              // → READY
    ...
    assert(has_action(APP_ACT_UI_PANEL_ON));
    reduce_btn(APP_EV_KEY_RELEASE, APP_BTN_UP, now + 11);
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_UP, now + 12);              // 离线拒绝
    assert(s.state == APP_ST_READY);
    assert(has_action(APP_ACT_PLAY_TONE));
    assert(has_action(APP_ACT_UI_REFRESH));
    assert(s.toast[0] != '\0');
...
static void test_ptt_max_talk_watchdog(void) {
    ...
    s.link_up = true;
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_OK, now + 10);
    start_ptt_click(now + 20);   // 开录,不再给松开事件
...
static void test_screen_off_keys_pass_through(void) {
    // a. ▼ DOWN 单击(息屏):唤醒 + APP_KEY_PASTE 上行
    reset();
    ...
    assert(s.screen_on == true);
    assert(has_action(APP_ACT_UI_SCREEN_ON));
    reduce_btn(APP_EV_KEY_CLICK, APP_BTN_DOWN, now + 20);       // 单击:照常粘贴
    app_action_t *a = find_action(APP_ACT_SEND_KEY_ACTION);
    assert(a && a->u.key_action.action == APP_KEY_PASTE);
    ...
    assert(s.screen_on == false);
    start_ptt_click(now + 10);         // 按下:唤醒 + 开录
    assert(s.screen_on == true);
    assert(s.state == APP_ST_LISTENING);
    assert(has_action(APP_ACT_SEND_VOICE_START));
...
int main(void) {
    ...
    test_ok_long_sends_enter();
    test_ptt_click_toggle();
