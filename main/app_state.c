...
// ---- 键位:单击 UP = 启动语音,语音中任意键 = 结束;单击 DOWN = 粘贴,长按 DOWN = 清空,单击 OK = 回车(2026-09-10 WeChat IME 版)----
...
// 所以清空改挂 DOWN 长按(0.5s,与 OK 锁屏同阈值):DOWN 上「单击=粘贴 / 长按=
...
// 按键动作(paste/enter/clear):上行给 PC client 执行注入。
...
static void handle_key(app_state_t *s, const app_event_t *ev, uint64_t now_ms,
    ...
    if (s->locked) {
        ...
        // OK 长按不再锁屏:OK 专用于发送回车,避免“回车后黑屏”的交互冲突。
    }

    // 点击式 PTT:开始后任意键按下立即结束(不执行该键的粘贴/回车语义)。
    // PRESS 回调发生在 CLICK 启动之后;如果 PRESS 丢失,RELEASE 仍在 LISTENING
    // 分支兜底。锁定时也允许用实体键结束录音。
    if (s->state == APP_ST_LISTENING && ev->type == APP_EV_KEY_PRESS &&
        !key_ev_is_fake(ev)) {
    ...
    // DOWN(音量减)长按 0.5s = 清空输入框(仅在非 LISTENING 态)。
    // 与同一颗键的"单击 = 粘贴"天然互斥:iot_button 一旦判成长按,这一按就只报
    // LONG_PRESS_START/LONG_PRESS_UP,不再补报 SINGLE_CLICK。
    ...
    switch (s->state) {
    case APP_ST_HOME:
        if ((ev->type == APP_EV_KEY_CLICK || ev->type == APP_EV_KEY_LONG_UP) &&
            b == APP_BTN_OK) {
            go_ready(s, now_ms, out, n, max);
        } else if (ev->type == APP_EV_KEY_CLICK && b == APP_BTN_DOWN) {
            send_key_action(s, APP_KEY_PASTE, out, n, max);
        }
        break;
    ...
    case APP_ST_READY:
        if (ev->type == APP_EV_KEY_CLICK && b == APP_BTN_DOWN) {
            send_key_action(s, APP_KEY_PASTE, out, n, max);
        } else if ((ev->type == APP_EV_KEY_CLICK || ev->type == APP_EV_KEY_LONG_UP) &&
                   b == APP_BTN_OK) {
            send_key_action(s, APP_KEY_ENTER, out, n, max);
        } else if (ev->type == APP_EV_KEY_CLICK && b == APP_BTN_UP) {
            // 真假单击只能从紧邻的 PRESS 读数区分。
            if (s->last_up_press_mv > PTT_UP_PRESS_MAX_MV ||
                now_ms - s->last_up_press_ms > 1000) {
                break;
            }
            // USB/BLE 握手后短暂抑制射频腐蚀造成的假 UP 单击。
    ...
    case APP_ST_LISTENING:
        // PRESS 分支已是主路径;这里保留 RELEASE 兜底,防止个别驱动丢 PRESS。
        if (ev->type == APP_EV_KEY_RELEASE || ev->type == APP_EV_KEY_LONG_UP) {
            if (now_ms - s->state_since_ms < PTT_MIN_TALK_MS) {
                app_action_t c = { .type = APP_ACT_STREAM_CANCEL };
                emit(out, n, max, c);
                app_action_t v = { .type = APP_ACT_SEND_VOICE_END };
