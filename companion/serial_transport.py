...
def _wmi_query_cdc_device_id():
    """在独立线程里查 WMI(见 discover_direct_cdc_path 的 COM 说明)。"""
    result: list[str] = []
    ...
    def run() -> None:
        try:
            ...
            pythoncom.CoInitialize()          # 只影响本线程
            try:
                wmi = win32com.client.GetObject("winmgmts:")
                for item in wmi.ExecQuery("SELECT DeviceID FROM Win32_PnPEntity"):
                    device_id = str(getattr(item, "DeviceID", "") or "")
                    if device_id.upper().startswith(r"USB\VID_303A&PID_1001&MI_00"):
                        result.append(device_id)
                # 必须先释放 COM 代理对象再 CoUninitialize,否则 pywin32 在退出时
                # 打印 "Win32 exception occurred releasing IUnknown"
                del item
                del wmi
                gc.collect()
            finally:
    ...
    worker = threading.Thread(target=run, name="wmi-cdc-lookup", daemon=True)
    worker.start()
    worker.join(timeout=8.0)
    return result[0] if result else None


def discover_direct_cdc_path():
    """Return the Windows device-interface path for MI_00 CDC/ACM.

    Some Windows installations have a stale Bluetooth modem claiming the same
    COM number as the Espressif CDC interface.  Opening the device interface
    directly bypasses the COM-number collision.

    COM apartment 说明(2026-09-11 实测):win32com 的 WMI 查询会把**调用线程**
    设成 STA,之后同进程里的 WinRT/BLE(bleak)会直接报
    "Thread is configured for Windows GUI but callbacks are not working"。
    所以 WMI 只在一次性工作线程里做,主线程/事件循环保持 MTA。
    """
    device_id = _wmi_query_cdc_device_id()
    if not device_id:
        return None
    normalized = device_id.lower().replace("\\", "#")
    return "\\\\?\\" + normalized + "#{86e0d1e0-8089-11d0-9ce4-08003e301f73}"
...
class SerialTransport:
    ...
    async def scan_for_device(self, name, timeout):
        ...
        if direct:
            print(f"[usb] 使用 USB 设备接口路径: {direct}")
            return direct
    ...
    async def connect(self, address, on_disconnect=None):
        ...
        try:
            self._ser = serial.Serial(address, BAUDRATE, timeout=0.1)
    ...
    def _read_loop(self):
        ...
        """
        try:
            # Do not use fileno()+select(): Windows device-interface paths can
            # be opened by pyserial but have no OS fd suitable for select().
            # A short blocking read timeout works for both COM symbols and
            # direct CDC interface paths.
