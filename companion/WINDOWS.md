> 注意:本文件写于 2026-08,描述的是 macOS/CLI relay 的路径(config.local.json、
> relay.py、mode ble|usb 互斥切换)。那套模式切换架构已退役 —— 现在是双通道常开,
> 文末的验收清单也仍是未跑状态。
> Windows + 微信输入法请走 [QUICKSTART.md](../QUICKSTART.md):
> install.cmd → doctor.cmd → open-console.cmd,桥接入口是 companion/wechat_bridge.py
