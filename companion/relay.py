...
class BleakTransport:
    ...
    async def _ensure_encrypted_link(self, retries=2):
        ...
        WinRT 重连不会自动启用加密: 不显式 pair() 时首个 CTRL 写会被拒
        (5, 'GATT Protocol Error: Insufficient Authentication'), 连接随即
        被系统取消(0x800704C7)。已绑定设备上 pair() 很快返回;失败重试一次
        后仍失败则放行,让上层按原有错误路径报告。
        """
        if sys.platform != "win32" or not hasattr(self._client, "pair"):
            return
        last = None
        for attempt in range(1, max(1, retries) + 1):
            try:
                await self._client.pair()
                print(f"[ble] 链路已配对/加密(第 {attempt} 次)")
                return
            except Exception as e:  # noqa: BLE001 - 平台异常类型不稳定
                last = e
                await asyncio.sleep(0.5)
        print(f"[ble] 配对未成功, 继续尝试连接: {type(last).__name__}: {last}",
              file=sys.stderr)
...
class Relay:
    ...
    def __init__(self, transport=None, *, asr_factory=None, inject_fn=None,
        ...
        # input backend. This bridge supplies its own virtual-mic ASR.
        if asr_factory is None:
            from asr_client import StreamingASR
            asr_factory = lambda: StreamingASR()
        if inject_fn is None:
            from inject import paste_text
            inject_fn = paste_text
        if key_action_fn is None:
        ...
        self._key_action_fn = key_action_fn
    ...
    async def run(self, device_addr=None, console_stdin=True):
        ...
        print("[relay] 等待语音(单击 UP 启动; 任意键结束; Ctrl+C 退出)")
    ...
    async def _on_key_action(self, action):
        ...
        """
        if action not in ("paste", "enter", "clear", "escape"):
