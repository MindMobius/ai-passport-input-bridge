...
新电脑 + 已刷固件的设备:**双击 `setup.cmd`** —— 它自动找/装 Python、建虚拟环境装依赖、
检测虚拟声卡(没装就下载并拉起官方安装程序)、最后打开控制台;唯一要你动手的是 UAC 授权
和装完虚拟声卡后重启一次。

想自己分步走看 **[QUICKSTART.md](QUICKSTART.md)**(4 步一次性 + 4 步日常)。
...
| 一键安装 | `setup.cmd` / `install.cmd` | 新电脑自检 + 装环境(Python/依赖/虚拟声卡)+ 开控制台 |
...
**待改进**:BLE 会话仍有约 10~15% 的通知丢在 **Windows 蓝牙主机栈**里 —— 两端对账可见:
设备 `drops: audio 0`(每一片都拿到了链路层回执),PC 侧只收到 82/100 片,差额既不在设备、
也不在桥接代码里(桥接的队列丢弃单独立账)。这一层改不动,绕法两种:插线走 USB(推荐),
或把音频从 ATT notify 改成 indication(带确认、会重传,但吞吐余量变小)——**后者未实施**。

本轮定位并修掉的三条主因(2026-09-11):

- 电脑端虚拟声卡输出缓冲过小:写一块 100ms 音频要阻塞 ~97ms,消费端几乎跑不过实时,队列一抖就丢。
  现为 `output_latency_s`(默认 0.4s 缓冲),同一测法单块 97.4ms → **2.4ms**;
- 固件 BLE 发送暂存固定 256B(MTU ≤ 256 时代遗留),协商到 517 后每块仍拆 4 片;
  现按协商上限开(每块 2 片);
- 回退了 2M PHY 请求(本机适配器会 1M↔2M 反复跳,跳变附近丢通知)。

USB 通道真人语音对照尚未跑过(此前链路测试 `audio_drops=0`)。
